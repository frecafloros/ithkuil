...
## 5.2 法(mood)
```  
我々が一般的に想像する法は多分illocution(5.3節)にも結構またがる  
```  
殆どの言語には「法」と呼ばれる動詞の形態論的分類があり、行為、状態、出来事、または事実性の程度に関する特定の態度や見解を示す役割を果たす。西洋言語に共通する法の例としては、直接法（事実の言及）、接続法（疑い、可能性、「かもしれない」）、命令法（命令、「今すぐ行け！」、「我々のために歌え！」など）、条件法（仮定、「もしできるなら…」）、希求法（願望、希望、予想、「彼が行ったことを願う」、「彼はここにいたと思う」など）、勧奨法（奨励、「彼が100年生きられますように！」、「自分の目で確かめてみよう！」など）。  
  
### 5.2.1 事実 (factual)
### 5.2.2 接続 (subjunctive)
### 5.2.3 仮定 (assumptive)
### 5.2.4 思索 (speculative)
### 5.2.5 反証 (counterfactive)
### 5.2.6 仮説 (hypothetical)
### 5.2.7 暗示 (implicative)
### 5.2.8 帰属 (ascriptive)
基礎となる前提条件から導き出された推論の事実性が不確かであることを除いて、すぐ上のimplicative moodと同じように機能する。  
  
## 5.3 illocution
### 5.3.1 断定 (assertive)
現実世界のある行為、出来事、または状態を説明したり、名前を付けたりすることを目的とした命題を表現するために使用され、聞き手をその命題の真実にコミットさせることを目的としている。assertive illocutionにおける発言とは、信じることも信じないこともできるものであり、真か偽かのどちらかである。このような発言には、一般的な記述、説明、説明が含まれる。
...
高調で表記される。予想される結果を達成する、または達成しようとしている行為、条件、事象、すなわち、何らかの目的、結果、最終的な状態の達成に向けて指向されたものを表す。このような区別は、西洋の言語では通常単語の選択によって処理される。以下はprocessualからcompletiveへの変化の例：  
